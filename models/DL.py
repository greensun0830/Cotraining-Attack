...
class PMF:
    ...
    def train(self, dataset, attack_size, fillter_size, nb_epochs=10):
        batch_size = 2048
        ...
        import time
        for cur_attack in range(self.dataset.origin_num_users, self.dataset.origin_num_users + attack_size):
            ...
            rate = self.sess.run(
                tf.sigmoid(
                    tf.matmul(self.user_embeddings[cur_attack:cur_attack + 1], tf.transpose(self.item_embeddings))))[0]
            m_rate = select_p * rate
            index = np.argsort(-m_rate)[:fillter_size]
            select_p[index] *= 0.9
            cur_user = np.zeros((1, dataset.num_items))
            cur_user[0, index] = np.clip(np.round(rate * dataset.max_rate) / dataset.max_rate, 0, 1)[index]

            cur_user[:, FLAGS.target_item] = 1.
            train_matrix = sp.vstack([train_matrix, sp.csr_matrix(cur_user)]).todok()
            print(cur_attack - self.dataset.origin_num_users, np.mean(cur_user > 0))
